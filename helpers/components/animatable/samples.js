...
function initAnim() {
	$('.animatable').each(function (i, el) {
		...
		var obj = {
			...
			startPoint:
				el.data('start-point') === undefined ? 0.8 : el.data('start-point'),
			shiftFactor:
				el.data('shift-factor') === undefined ? 0.1 : el.data('shift-factor'),
			animIndex:
				el.data('anim-index') === undefined ? undefined : el.data('anim-index'),
			startOnView:
				el.data('start-onview') === undefined
					? 2
					: parseFloat(el.data('start-onview')),
			animVariety:
				el.data('anim-variety') === undefined
					? 'default'
					: el.data('anim-variety'),
			progress:
				el.data('progress-anim') === undefined
					? false
					: el.data('progress-anim'),
			tl: false,
		};
		...
		if (pastHero_$scrollTop !== $scrollTop) {
			$.each($allAnimateElements, function (key, val) {
				if (val.progress) {
					...
					if (
						!val.shown &&
						$scrollTop + windowHeight * val.startPoint > val.position
					) {
						val.shown = true;
...
if ($allAnimateElements) {
	$.each($allAnimateElements, function (key, value) {
		...
		value.height = value.el.hasClass('parallaxElement')
			? windowHeight
